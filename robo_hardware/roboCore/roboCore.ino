...
Encoder myEncoder(encoderB, encoderA);
  ...
  if (timer != NULL)
  {
    if (preLimit != lim_switch())
    {
      ...
      if (lim_switch() == false)
      {
        ...
        myEncoder.write(0);
        genaral_msg.linear.y = state_running;
      }
    }
    ...
    odom_msg.header.stamp.sec = (uint32_t)(millis() / 1000000);
    odom_msg.header.stamp.nanosec = (uint32_t)(millis() % 1000000) * 1000;

    unsigned long now = millis();
    float vel_dt = (now - prev_odom_update) / 1000.0;
    prev_odom_update = now;

    odom_msg.pose.pose.position.x = pos_x + (0.5 * imu_msg.linear_acceleration.x * vel_dt * vel_dt);
    odom_msg.pose.pose.position.y = pos_y + (0.5 * imu_msg.linear_acceleration.y * vel_dt * vel_dt);
    odom_msg.pose.pose.position.z = 0.0;

    float delta_heading = imu_msg.angular_velocity.z * vel_dt;
    ...
    odom_msg.twist.twist.linear.x = sqrt(2 * imu_msg.linear_acceleration.x * vel_dt);
    odom_msg.twist.twist.linear.y = sqrt(2 * imu_msg.linear_acceleration.y * vel_dt);
    ...
    genaral_msg.linear.x = float(myEncoder.read());
    genaral_msg.linear.z = state_drive;
